...
//Funktioner som tar hand om beställningar. 
...
function get_betalningssida_med_kundinmatning() {//Funktion för när man beställer utan att logga in. 
    ...
    <p><label for="postort">City: </label>
    <br>
    ...
    <a href=log_in.php>Already have an account? Log in here</a>
</div>
</section>
	';
...
function ny_kund($fnamn,$enamn,$adress,$telefonnummer,$email,$pass,$postnummer,$postort) {
	...
	$sql = "INSERT INTO kund(fnamn, enamn, adress, telefonnummer, email, pass, postnummer, postort) VALUES ("
		."'".$fnamn."',"  
		."'".$enamn."',"        
		."'".$adress."'," 
		."'".$telefonnummer."',"
...
function get_betalningssida() {
		...
		return "<section id='sista'><h1>Click on Order to place an order</h1><form id='bestall' method=POST action=''><input type=submit value='Order'/><input type=hidden name='bestallninggjord'/></form></section>";
...
if(count($_SESSION['kundkorg'])==0) {
	return "";
		...
		if($_SESSION['access']=="admin") {
			...
			return "<section><form id='bestall1' method=POST action=''><input type=submit value='Place order'/><input type=hidden name='fragaombestallning'/></form></section>";
	}
	}
