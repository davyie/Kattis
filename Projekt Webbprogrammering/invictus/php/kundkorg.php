...
2016-01-08
*/
//Denna tar hand om kundkorg och lägger till varor. 
...
function get_kundkorg_kassa() {
	...
	if(count($_SESSION['kundkorg'])==0) {
		echo "<p>The cart is empty. <a href='watches.php'>Go to watches to get your very own Invictus watch</a></p>";
	} else {
    ...
    <form class='tabort' action=\"".htmlentities($_SERVER['PHP_SELF'])."\" method=POST><input type=submit id='tabort' value='Remove from cart'/><input type=hidden name=tabortfrankundkorg value=".$key."/></form>
    </div>";
			}
